    ...
    def event_loop_overlap(self):
        ...
        if enable_profiling:
            ...
            prof = torch_npu.profiler.profile(
                ...
                schedule=torch_npu.profiler.schedule(wait=1, warmup=1, active=50, repeat=1, skip_first=1),
                record_shapes=True,
                profile_memory=True,
                with_stack=True,
                with_flops=False,
        ...
        while True:
            ...
            if batch:
                if enable_profiling:
                    prof_cnt += 1
                    print(f"{prof_cnt=}",flush=True)
                    if prof_cnt == prof_start:
                        prof.start()
                    if prof_cnt == prof_start + 50:
                        prof.stop()
                batch_result = self.run_batch(batch)
                self.result_queue.append((batch.copy(), batch_result))

                if enable_profiling and prof_cnt > prof_start and prof_cnt < prof_start + 50:
                    prof.step()
            if self.last_batch:
