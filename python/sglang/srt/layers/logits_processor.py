...
class LogitsMetadata:
    ...
    def from_forward_batch(cls, forward_batch: ForwardBatch):
        if (
            ForwardMode.is_extend(forward_batch.forward_mode)
            and forward_batch.return_logprob
            and not ForwardMode.is_target_verify(forward_batch.forward_mode)
        ):
...
class LogitsProcessor(nn.Module):
    ...
    def forward(
        ...
        if (
            ForwardMode.is_decode_or_idle(logits_metadata.forward_mode)
            or ForwardMode.is_target_verify(logits_metadata.forward_mode)
        ):
            ...
            ForwardMode.is_extend(logits_metadata.forward_mode)
        ...
        if CaptureHiddenMode.need_capture(logits_metadata.capture_hidden_mode):
            if CaptureHiddenMode.is_full(logits_metadata.capture_hidden_mode):
                if aux_hidden_states is not None:
                    aux_hidden_states = torch.cat(aux_hidden_states, dim=-1)
                    hidden_states_to_store = aux_hidden_states
                else:
                    hidden_states_to_store = hidden_states
            elif CaptureHiddenMode.is_last(logits_metadata.capture_hidden_mode):
