...
class Qwen3_VisionTransformer(nn.Module):
    ...
    def fast_pos_embed_interpolate(self, grid_thw):
        ...
        for t, h, w in grid_thw:
            pos_embed = torch.arange(num_grid_per_side*num_grid_per_side).to(device=self.pos_embed.weight.device)
            pos_embed = self.pos_embed(pos_embed).permute(1, 0).unsqueeze(0).reshape(1, -1, num_grid_per_side, num_grid_per_side)
